...
#
# Requires:
#   RPM based system, DNF versionlock plugin
#
# Sample usage:
#   dnf::versionlock { '0:bash-4.1.2-9.el6_2.*':
#     ensure  => 'present',
#   }
#
define dnf::versionlock (
  ...
  contain dnf::plugin::versionlock

  unless $name.is_a(Dnf::VersionlockString) {
    fail('Package name must be formated as %{EPOCH}:%{NAME}-%{VERSION}-%{RELEASE}.%{ARCH}. See Dnf::Versionlock documentation for details.')
  }
  ...
  case $ensure {
    'present', 'exclude', default: {
      concat::fragment { "dnf-versionlock-${name}":
        content => "${line_prefix}${name}\n",
        target  => $dnf::plugin::versionlock::path,
      }
    }
    'absent':{
